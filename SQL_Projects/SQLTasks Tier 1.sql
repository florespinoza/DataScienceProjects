...
/* --------------------------------------------------------------------- */
/* Q3: Write an SQL query to show a list of facilities that charge a fee 
to members, where the fee is less than 20% of the facility's monthly 
...
SELECT concat(m.firstname, ' ', m.surname) AS name, f.name AS facility, b.starttime,
...
SELECT name, facility, cost
 FROM (SELECT concat(m.firstname, ' ', m.surname) AS name, f.name AS facility, 
       CASE WHEN b.memid = 0 THEN b.slots*f.guestcost
       ELSE b.slots*f.membercost END AS cost
       FROM Bookings AS b
       INNER JOIN Facilities as f
...
You'll need to paste your query into value of the 'query1' variable and run 
the code block again to get an output.
...
SELECT s.member AS member, concat(m.surname, ' ', m.firstname) as recommendedby
FROM Members as m
INNER JOIN (
 SELECT concat(surname, ' ', firstname)as member, recommendedby AS memid
 FROM Members
